...
** @sections@ - a :symbol array of sections (must map to named routes)
...
* @with@ - specifies the method to use to authorize against (defaults to @logged_in?@ method)
...
* @authorized_css@ - specifies the css class that goes on all authorized tabs (defaults to 'authorized_nav_link')

h2. Get it at GitHub

<pre><code>git clone git://github.com/rpheath/navigation_helper.git</code></pre>
